...
//
// Preview-only additions below are Vercel's own documented requirements for
// its Toolbar/Live Feedback overlay (https://vercel.com/docs/vercel-toolbar/managing-toolbar#using-a-content-security-policy),
// which only ever injects on preview deployments — production visitors
// never load vercel.live at all, so there's no reason to widen the
// production policy to accommodate it.
...
function csp(nonce: string) {
  ...
  if (isPreview) {
    ...
    directives["img-src"].push("https://vercel.live", "https://vercel.com");
